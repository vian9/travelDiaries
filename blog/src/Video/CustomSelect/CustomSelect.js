  ...
  return <div className="dropdown-container">
    ...
    <div className="dropdown-input">
      ...
      <div className='dropdown-values'>
        {value.length ? value.map(v => <div key={v} className='dropdown-value'> {options[v].label} 
        <span className='dropdown-remove' onClick={() => removeValue(v)}>X</span></div>) : <div onClick={() => setIsActive(!isActive)} 
        className='dropdown-placeholder'>Select your Domain</div> }
      </div>
    </div>
    <div className={classnames('dropdown-options', { 'dropdown-active': isActive })}>
      {options.filter(i => value.findIndex(v => v === i.id) === -1).map(item =>
        <div onClick={() =>{ 
          ...
          }} className="dropdown-item" key={item.id}>
          <img src={item.logo}/>
          {item.label}
        </div>
      )}
